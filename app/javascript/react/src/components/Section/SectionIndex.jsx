...
const Batches = ({ courseId }) => {
  const [batches, setBatches] = useState([]);
  const [newBatch, setNewBatch] = useState({ name: '', start_date: '', end_date: '' });
  const [editBatch, setEditBatch] = useState(null);

  // Fetch batches for a specific course
  useEffect(() => {
    axios.get(`/batches/batcheList/1868`)
      .then(response => {
  ...
  const handleCreateBatch = () => {
    axios.post('/batches/create', { mg_batch: newBatch })
      .then(response => {
        setBatches([...batches, response.data]);
        setNewBatch({ name: '', start_date: '', end_date: '' });
  ...
  const handleUpdateBatch = () => {
    axios.post(`/batches/update/${editBatch.id}`, { mg_batch: editBatch })
      .then(response => {
        const updatedBatches = batches.map(batch => batch.id === editBatch.id ? response.data : batch);
  ...
  const handleDeleteBatch = (batchId) => {
    axios.delete(`/batches/delete/${batchId}`)
      .then(() => {
  ...
  return (
    <div>
      ...
      <ul>
        {batches.map(batch => (
          <li key={batch.id}>
            {batch.name} - {batch.start_date} to {batch.end_date}
