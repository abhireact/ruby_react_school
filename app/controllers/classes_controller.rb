class ClassesController < ApplicationController
   ...
   @classes = MgCourse.where(mg_school_id: session[:current_user_school_id], is_deleted: 0, mg_time_table_id: @current_academic_year_id).order(:id)
   ...
   @academic_name = MgTimeTable.where(is_deleted: 0, mg_school_id: session[:current_user_school_id]).pluck(:name, 'CONCAT(mg_time_tables.start_date,"+",mg_time_tables.end_date,"+",mg_time_tables.id)')
   ...
   def create
     logger.info "Course Create Method"
       ...
       @time_table_details = params[:mg_time_table_id].split("+")
     # @course.mg_time_table_id = @time_table_details[2].to_i
 ...
 def modeltest
   ...
   def course_params
     params.require(:mg_course).permit(:course_name, :section_name, :code, :is_deleted, :grading_type, :mg_school_id, :mg_wing_id, :created_by, :updated_by, :index)
   end
