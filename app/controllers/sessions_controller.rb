...
def permanent_blocking
  ...
  def create
    ...
    if user.present?
      user_array=[ "student", "admin", "employee", "guardian", "principal", "school_admin", "", "Teaching Staff", "Non Teaching Staff", "chairman" ]
      puts "============================"
      ...
      if user_array.include? user.user_type
        ...
        if @user_type=="employee"
          teaching_employees=MgEmployee.find_by(is_deleted: 0, mg_school_id: session[:current_user_school_id],
            mg_user_id: user.id)
          teaching_employees=MgEmployeeCategory.find_by(is_deleted: 0, id: teaching_employees.mg_employee_category_id)
        ...
        puts @user_type
        puts "checkinggggggggg user_tyoeeeeeeeeeeeeeeeeeeee"
        if @user_type==params[:user_type]
          @login_access=1
        else
          @login_access=0
        end
      else
        if params[:user_type]=="functional incharge"
          @login_access=1
        else
          @login_access=0
        end
  ...
  puts "inside else for cloud admin"
    ...
    if guardian.has_login_access
      student_details = MgStudent.find_by(id: guardian.mg_student_id, is_deleted: 0, mg_school_id: session[:current_user_school_id])
      if student_details.is_archive == false
        @tmp=1
      end
    end
  end
  ...
  if @tmp==1
    ...
    login_student=MgStudent.find_by(mg_user_id: session[:user_id], is_deleted: 0, mg_school_id: session[:current_user_school_id])
...
def send_email_create(user, msg)
    ...
    if @email_to.present?
      ...
      if @email_from.present?
        ...
        from_user_id = "jreddy@mindcomgroup.com"
      end
